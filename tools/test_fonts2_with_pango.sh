...
font_dir="$root/fonts2"
out_dir="${1:-$root/fonts2-pango-test-output}"
sample='אבגדהוזחטיכךלמםנןסעפףצץקרשת'

if [[ ! -d "$font_dir" ]]; then
  echo "missing $font_dir; run tools/uniformize_ancient_fonts.py first" >&2
  exit 1
fi
...
bad = []
for g in f.glyphs():
    if g.unicode >= 0 and not (0x0590 <= g.unicode <= 0x05FF):
...
' "$font" 2>/dev/null
  ...
  if [[ -n "$outside" ]]; then
    echo "FAIL $family: glyphs outside Hebrew block: $outside" >&2
    status=1
    continue
  fi
