...
\newcommand{\SourceRefs}{%
    \eJ{J}, %
    \eBookOfRecords{Records}, %
    \textit{Songs}%
}

\newcommand{\PriestSourceRefs}{%
    \eE{E}, %
    \eP{P}, %
    \DtrALabel, %
    \DtrBLabel, %
...
\newcommand{\LawRow}[2]{%
    {\LawFont #1} & {\raggedright\arraybackslash\LawFont #2}\\%
}

\newcommand{\LawRows}{%
    \LawRow{{\Aaronid Aaronid Laws}}{\Aaronid Leviticus; Exodus 25-40.}%
    \LawRow{{\Mushite Mushite Laws}}{\Mushite Covenant Code in Exodus; Deuteronomistic Law Code.}%
    \LawRow{\cG{J Laws}}{\cG{Rare. Possibly a joke.\textsuperscript{6}}}%
}

\newcommand{\LeviteBlend}{%
    {\Levite\Mushite Of the Levites in the South}%
}
...
\newcommand{\FontChoicePage}{%
    ...
    \noindent\begin{tabular}{@{}p{0.43\linewidth}p{0.51\linewidth}@{}}
        \hProtoA{אבגדהוזחטיכלמנסעפצקרשת} & \eProtoA{Early Levite poetry uses a fifteenth-century Proto-Sinaitic hand: older, rougher, and good for the Song of the Sea.}\\
        \hProto{אבגדהוזחטיכלמנסעפצקרשת} & \eProto{Early Israelite poetry uses the thirteenth-century Proto-Sinaitic hand: still old, less cave-wall-on-fire, and right for the Blessing of Jacob.}\\
        \hJ{אבגדהוזחטיכלמנסעפצקרשת} & \eJ{J is native to the land, non-bold, and in Paleo-Hebrew because J dates to around 922 B.C.}\\
        \hE{אבגדהוזחטיכלמנסעפצקרשת} & \eE{E is Mushite and Levite, but uses the cleaner Tel Dan Paleo-Hebrew hand. Less chaos, same priestly north/south argument.}\\
        \hP{אבגדהוזחטיכלמנסעפצקרשת} & \eP{P uses the Isaiah Scroll Paleo-Hebrew hand. The niqqud are anachronistic, but he would have loved them if the font gods had cooperated. Aaronids are all about the exact right way to do stuff. P loves the dots. P basically is the dots.}\\
        \hR{אבגדהוזחטיכלמנסעפצקרשת} & \eR{R uses Ezra square script with no niqqud because he is probably Ezra, after the Babylonian exile.}\\
        \hRecords{אבגדהוזחטיכלמנסעפצקרשת} & \eBookOfRecords{Records use Gezer Paleo-Hebrew and grey text: bureaucracy, accounting, begats. A spreadsheet wearing sandals.}\\
    \end{tabular}
    \endgroup
    \cleardoublepage
}
...
\newcommand{\SourceLegendPage}{%
    ...
    \noindent\begin{tabular}{@{}p{0.23\linewidth}p{0.71\linewidth}@{}}
        Regular
        & The People. We write regular. Bold is for the bold.\\
        {\Levite Bold} & {\Levite The Priests. Or the Levites. Same thing.}%
        \textsuperscript{1}%
        (\PriestSourceRefs.)\\

        \eR{Background} \eRJE{Background}
        & \eR{Redactors. Editors, occasionally write small amounts of glue prose to make multiple sources consistent,} \eRJE{while others write less and delete more from their sources.}\\[0.7em]
        \eJ{Green} & \eJ{J. The first author of the bible.\textsuperscript{2}
            The first author of prose on earth.\textsuperscript{3}
            The author of nearly all the most popular bible stories.\textsuperscript{4}
            The author in the bible with the best sense of humor.\textsuperscript{5}}\\
        {\Mushite Yellow} & {\Mushite Mushites. A group of Levite priests who claim descent from Moses. Associated with the first temple in the city of Shiloh.} (\MushiteSourceRefs.)\\
        {\Aaronid Blue} & {\Aaronid Aaronids. A group of Levite priests who claim descent from Aaron. Associated with the first and second temple in the city of Jerusalem.} (\eP{P}, \eR{R}.)\\
        {\DeuterB Orange} & {\DeuterB Deuteronomists. Mushites after the yellow has been left in the theological toaster.} (\DtrALabel, \DtrBLabel, \DtnLabel.)\\
        {\Record Grey} & {\Record Bureaucracy: records, accounting, begat lists, external documents. (Genesis 5, Genesis 14.)}\\[0.7em]
        {\Poetry Italics} & {\Poetry Songs, early poetry \LeviteBlend{} (Exodus 15), \cG{Israelites in the North} (Judges 5, Genesis 49), {\Mushite Deuteronomy 32}, and maybe prophecy when prophecy starts singing.}\\
        {\LawFont Mono} & {\LawFont All forms of code, including legal codes, for reasons that need not be explained.}\\
        \LawRows
    \end{tabular}
    \par\smallskip
    {\scriptsize
    \textsuperscript{1}According to the Mushites, but not the Aaronids. That'll make sense eventually.\par
    \textsuperscript{2}Aside from a few brief archaic songs.\par
    \textsuperscript{3}As far as we know.\par
    \textsuperscript{4}Not including the Jesus bits, though J's at least tied.\par
    \textsuperscript{5}And the most misunderstood.\par
    \textsuperscript{6}J is the funny one.\par
    }
    \endgroup
    \cleardoublepage
    \FontChoicePage
