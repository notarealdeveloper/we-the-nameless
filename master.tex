...
function mark_translit(s)
  ...
  return (s:gsub("[%z\1-\127\194-\244][\128-\191]*", function(c)
    if matres[c] then return "\\mTR{" .. c:lower() .. "}" end
    return c
  end))
end

function mark_hebrew(s)
  local matres = { ["י"]=true, ["ו"]=true } -- add ["ה"]=true if wanted
  return (s:gsub("[%z\1-\127\194-\244][\128-\191]*", function(c)
    if matres[c] then return "\\mHE{" .. c .. "}" end
    return c
  end))
end
}

\newcommand{\trM}[1]{\directlua{tex.sprint(mark_translit([[#1]]))}}
\newcommand{\hJM}[1]{\hJ{\directlua{tex.sprint(mark_hebrew([[#1]]))}}}
...
\newcommand{\PrimarySourceFontsPage}{%
    ...
    \LegendRow{\LegendFontSample{\hP{אבגדהוזחטיכלמנסעפצקרשת}}}{%
        \eP{
