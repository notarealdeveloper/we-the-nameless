...
% choices: none, or the basename of an image in img/covers/
\ifdefined\ConfigCover\else
    \def\ConfigCover{none}
\fi

% choices: simple, fancy
\ifdefined\ConfigCoverStyle\else
    \def\ConfigCoverStyle{simple}
\fi

% choices: nameless, ld
\ifdefined\ConfigSpine\else
    \def\ConfigSpine{ld}
\fi

\ifdefined\ConfigKDP\else
    \def\ConfigKDP{false}
\fi
\ifdefined\KDPAssetDir\else
    \def\KDPAssetDir{build/kdp-assets}
\fi
...
\def\ConfigThemeAssetSuffix{light}
\ifx\ConfigTheme\ConfigThemeDark
    \def\ConfigThemeAssetSuffix{dark}
\fi
...
\fi
\ifx\ConfigOutputMode\ConfigOutputModeTech
    \ConfigOutputModeTechtrue
\else
    \ConfigOutputModeTechfalse
\fi
...
\ifConfigOutputModeTech
% Tech keeps the same typographic ladder as 11pt book mode, scaled together.
% This preserves source/font relationships while making tech reading practical.
\TechModeTextSizes
...
\ifConfigOutputModeTech
    % Keep the existing 7in width; expand height to match a 3.07 x 6.44 device ratio.
    \usepackage[
        paperwidth=7in,
        paperheight=14.68in,
        left=0.625in,
...
% The alphabet-history page deliberately uses a custom size ladder.  Permit
% Computer Modern's legacy text and math fonts at those exact sizes instead
% of substituting the nearest traditional design size.
\usepackage{fix-cm}
\usepackage{amsmath}
\usepackage{titlesec}
\usepackage{tocloft}
...
\DeclareBookImageDir{1 On The Lamb}{10-dudetheyreontome/on-the-lamb/1-on-the-lamb}

\newcommand{\ImageNotFound}[1]{%
    \PackageError{WeTheNameless}{Image not found: #1}{%
        Looked under \CurrentBookImageDir/include for #1-\ConfigThemeAssetSuffix and #1, with jpg/png extensions.%
    }%
}
\newcommand{\imageordinary}[2][width=0.9\linewidth]{%
    \IfFileExists{\CurrentBookImageDir/include/#2-\ConfigThemeAssetSuffix.jpg}{%
        \includegraphics[#1]{\CurrentBookImageDir/include/#2-\ConfigThemeAssetSuffix.jpg}%
    }{%
        \IfFileExists{\CurrentBookImageDir/include/#2-\ConfigThemeAssetSuffix.png}{%
            \includegraphics[#1]{\CurrentBookImageDir/include/#2-\ConfigThemeAssetSuffix.png}%
        }{%
...
\newcommand{\image}[2][width=0.9\linewidth]{%
    \ifConfigKDPOn
        \IfFileExists{\KDPAssetDir/#2-\ConfigThemeAssetSuffix.jpg}{%
            \includegraphics[#1]{\KDPAssetDir/#2-\ConfigThemeAssetSuffix.jpg}%
        }{%
            \IfFileExists{\KDPAssetDir/#2.png}{%
                \includegraphics[#1]{\KDPAssetDir/#2.png}%
            }{\imageordinary[#1]{#2}}%
        }%
    \else
        \imageordinary[#1]{#2}%
...
\let\cyrillicfont\CyrillicFont
\DeclareTextFontCommand{\cyr}{\cyrillicfont}
...
\newfontfamily\NewGardinerFont[
    ...
    Renderer=HarfBuzz,
    Scale=1.9
]{egyptian-hieroglyphs-new-gardiner.ttf}
...
\newfontfamily\PaleoHebrewFont[
    Path=\LocalFontPath,
    Renderer=HarfBuzz,
    BoldFont=babel-paleo-hebrew-phoenician.ttf,
    Scale=1.22
]{babel-paleo-hebrew-phoenician.ttf}
...
]{noto-sans-canadian-aboriginal.ttf}

% Hitties speak in Anatolian / Luwian hieroglyphs
\newfontfamily\AnatolianFont[
    Path=\LocalFontPath,
    Renderer=HarfBuzz,
    Scale=1.50
]{noto-sans-anatolian-hieroglyphs.ttf}

% Reusable font catalog. Source profiles below bind these fonts to J/E/P/etc.
\newfontfamily\EnglishFont[
	\NamelessTextFallbackFeature
]{FreeSerif}
...
\ifConfigKDPOn
    % KDP asks graphic/table rules to be at least 0.75 pt.
    \setlength{\VerseRuleWidth}{0.75pt}
\fi
...
% visually match the Hebrew face in the lite theme.
...
\definecolor{TechLinkColor}{HTML}{8ec7ff}
...
\ifx\ConfigTheme\ConfigThemeDark
    ...
    \colorlet{ContentsLinkColor}{TechLinkColor}
\else
    % Match \aC text exactly in lite mode.
...
% hittites
\newcommand{\anatolian}[1]{%
    {\leavevmode\textdir TLT\AnatolianFont #1}%
}
...
\DeclareSourceProfile{Proto}
	{\EnglishProtoFont}			{\color{ProtoColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\PaleoASCIITelZayit}		{\color{ProtoColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoA}
	{\EnglishProtoFont}			{\color{ProtoAColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoAFont}				{\color{ProtoAColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoB}
	{\EnglishProtoFont}			{\color{ProtoBColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoBFont}				{\color{ProtoBColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoC}
	{\EnglishProtoFont}			{\color{ProtoCColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoCFont}				{\color{ProtoCColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoD}
	{\EnglishProtoFont}			{\color{ProtoDColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoDFont}				{\color{ProtoDColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoE}
	{\EnglishProtoFont}			{\color{ProtoEColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoEFont}				{\color{ProtoEColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoF}
	{\EnglishProtoFont}			{\color{ProtoFColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoFFont}				{\color{ProtoFColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{ProtoG}
	{\EnglishProtoFont}			{\color{ProtoGColor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\ProtoGFont}				{\color{ProtoGColor}\ProtoSinaiticASCIIWeight}

\DeclareSourceProfile{Paleo}
	{\EnglishProtoFont}			{\color{PaleoColor}\mdseries\addfontfeatures{Ligatures=TeX}}
...
\DeclareSourceProfile{J}
	{\EnglishFont}				{\SourceBackground{Jbgcolor}\Yahwist\mdseries\addfontfeatures{Ligatures=TeX}}
	{\PaleoHebrewPhoenician}	{\SourceBackground{Jbgcolor}\Yahwist\mdseries}

\DeclareSourceProfile{JP}
	{\EnglishFont}				{\color{JPcolor}\PriestStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewEzra}				{\color{JPcolor}\addfontfeatures{Scale=1.40}\mdseries}

\DeclareSourceProfile{JPP}
	{\EnglishFont}				{\color{JPPcolor}\PriestStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewEzraRaw}			{\color{JPPcolor}\addfontfeatures{Scale=1.40}\mdseries}

\DeclareSourceProfile{PR}
	{\EnglishFont}				{\SourceBackground{Rbgcolor}\color{Pcolor}\Redactor\bfseries\addfontfeatures{Ligatures=TeX}}
	{\HebrewEzraRaw}			{\SourceBackground{Rbgcolor}\color{Pcolor}\Redactor\addfontfeatures{Scale=1.40}\mdseries}

\DeclareSourceProfile{JE}
	{\EnglishFont}				{\color{JEcolor}\bfseries\addfontfeatures{Ligatures=TeX}}
	{\PaleoASCIIStandard}		{\color{JEcolor}\addfontfeatures{Scale=1.35}\mdseries}

% Mushites

\DeclareSourceProfile{E}
	{\EnglishFont}				{\addfontfeatures{Scale=0.96}\SourceBackground{Ebgcolor}\MushiteStyle\PriestStyle\addfontfeatures{Ligatures=TeX}}
...
\DeclareSourceProfile{DtrA}
	{\EnglishFont}				{\DeuteronomistAStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavid}				{\HebrewDavidLarge\DeuteronomistAStyle}

\DeclareSourceProfile{DtrB}
	{\EnglishFont}				{\DeuteronomistBStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavid}				{\HebrewDavidLarge\DeuteronomistBStyle}

\DeclareSourceProfile{DtrH}
	{\EnglishFont}				{\color{Dtr1color}\SourceBackground{Rbgcolor}\Redactor\mdseries\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavid}				{\HebrewDavidLarge\color{Dtr1color}\SourceBackground{Rbgcolor}\Redactor\mdseries}

\DeclareSourceProfile{Dtn}
	{\LawFont}					{\DeuteronomicLawStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavid}				{\HebrewDavidLarge\DeuteronomicLawStyle}

% Aaronids

\DeclareSourceProfile{P}
	{\EnglishFont}				{\SourceBackground{Pbgcolor}\AaronidStyle\PriestStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewEzraRaw}			{\SourceBackground{Pbgcolor}\AaronidStyle\addfontfeatures{Scale=1.40}\mdseries}
	% Don't change this font scale!

% Json: speaker B's red with the priestly bold weight.
\DeclareSourceProfile{Json}
	{\EnglishFont}				{\color{Redcolor}\PriestStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavidRaw}			{\color{Redcolor}\PriestStyle}

\DeclareSourceProfile{EP}
	{\EnglishFont}				{\color{EPcolor}\PriestStyle\addfontfeatures{Ligatures=TeX}}
	{\HebrewEzra}				{\color{EPcolor}\addfontfeatures{Scale=1.40}\mdseries}

\DeclareSourceProfile{EPP}
	{\EnglishFont}				{\color{EPPcolor}\PriestStyle\addfontfeatures{Ligatures=TeX}}
...
% Currently identical to R, but kept as a separate profile so redactor C can
% diverge without changing R.
\DeclareSourceProfile{RC}
	{\EnglishFont}				{\SourceBackground{Rbgcolor}\AaronidStyle\Redactor\addfontfeatures{Ligatures=TeX}\mdseries}
	{\HebrewEzraRaw}			{\SourceBackground{Rbgcolor}\AaronidStyle\Redactor\addfontfeatures{Scale=1.40}\mdseries}
	% Don't change this font scale!
...
\DeclareSourceProfile{Diff}
	{\EnglishFont}				{\SourceBackground{Rbgcolor}\Redactor\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavidRaw}			{\SourceBackground{Rbgcolor}\Redactor}

\DeclareSourceProfile{RB}
...
\DeclareSourceProfile{X}
	{\EnglishFont}				{\color{Xcolor}\mdseries\addfontfeatures{Ligatures=TeX}}
	{\HebrewDavid}				{\HebrewDavidLarge\color{Xcolor}\mdseries}

\DeclareSourceProfile{Aramaic}
	{\EnglishFont}				{\color{TextColor}\mdseries\addfontfeatures{Ligatures=TeX}}
...
\DeclareSourceProfile{Records}
	{\EnglishFont}				{\SourceBackground{Recordsbgcolor}\Records\addfontfeatures{Ligatures=TeX}}
	{\PaleoHebrewMono}			{\SourceBackground{Recordsbgcolor}\Records}

\DeclareSourceProfile{Other}
	{\EnglishFont}				{\SourceBackground{Otherbgcolor}\EnglishOtherSource\addfontfeatures{Ligatures=TeX}}
...
\newcommand{\eRA}[1]{\ApplyEnglishSourceProfile{\estyleRA}{#1}}
\newcommand{\eRB}[1]{\ApplyEnglishSourceProfile{\estyleRB}{#1}}
\newcommand{\eRC}[1]{\ApplyEnglishSourceProfile{\estyleRC}{#1}}
...
\newcommand{\hRA}[1]{\hRARenderer{\hstyleRA}{#1}}
\newcommand{\hRB}[1]{\hRBRenderer{\hstyleRB}{#1}}
\newcommand{\hRC}[1]{\hRCRenderer{\hstyleRC}{#1}}
\newcommand{\hRJE}[1]{\hRJERenderer{\hstyleRJE}{#1}}
\let\hI\hRJE
\newcommand{\sR}[1]{\ApplyEnglishSourceProfile{\estyleR}{#1}}
\newcommand{\sRA}[1]{\ApplyEnglishSourceProfile{\estyleRA}{#1}}
\newcommand{\sRB}[1]{\ApplyEnglishSourceProfile{\estyleRB}{#1}}
\newcommand{\sRC}[1]{\ApplyEnglishSourceProfile{\estyleRC}{#1}}
...
\newcount\SourceFontLineBreakTechPenalty
\SourceFontLineBreakTechPenalty=3000
\newcommand{\SourceFontLineBreak}{%
    \ifConfigOutputModeTech
        \ifhmode
            \unskip
            \penalty\SourceFontLineBreakTechPenalty
...
% The image is centered and scaled to fill the physical page; any excess is
% cropped at the page edge.  ConfigCover=none suppresses the page entirely.
\newcommand{\WeCoverPage}{%
    \ifx\ConfigCover\ConfigCoverNone
    \else
        \clearpage
        \thispagestyle{empty}%
        \begin{tikzpicture}[remember picture,overlay]
            \node[inner sep=0pt] at (current page.center) {%
                \includegraphics[width=\paperwidth]{img/covers/\ConfigCover}%
            };
        \end{tikzpicture}%
        \null
        \clearpage
...
\newcommand{\AlphabetHistoryTextSizes}{%
    ...
    % produce the same clean alignment that tech mode has.
...
\DeclareChapterLinkRange{1 On The Lamb}{3}
\DeclareChapterLinkRange{2 On The Lamb}{3}
\DeclareChapterLinkRange{3 On The Lamb}{2}
\DeclareChapterLinkRange{Esther}{10}
\DeclareChapterLinkRange{Dudetheyreontome}{1}
\DeclareChapterLinkRange{1 NHS}{1}
\DeclareChapterLinkList{Sects / Acts}{%
    \par\noindent\hspace*{1em}%
    \hyperlink{chapter.1 NHS.1}{\ChapterLinkText{1 NHS: The Nth Stair}}%
}
...
\DeclareBookExtraLinkList{Exodus}{%
    \ifConfigApocryphaOn
        \BookLink{Exodus Apocrypha}{Exodus Apocrypha}%
    \fi
}
...
\DeclareBookChapterLinkList{Dudetheyreontome}{%
    \BookPartChapterLinks{Dudetheyreontome}%
    \BookPartChapterLinks{1 On The Lamb}%
    \BookPartChapterLinks{Esther}%
    \BookPartChapterLinks{2 On The Lamb}%
    \BookPartChapterLinks{Ezra}%
    \BookPartChapterLinks{3 On The Lamb}%
    \BookPartChapterLinks{Nehemiah}%
    \BookPartChapterLinks{Sects / Acts}%
    \EpistlesContents%
}
...
\DeclareBookChapterSummaryLinkList{Genesis}{%
    ...
    \ChapterSummaryLink{Genesis}{35}{Genesis}{Ben the last; Miss Ewe}%
    \ChapterSummaryLink{Genesis}{36}{Genesis}{Red line}%
    \ChapterSummaryLink{Genesis}{37}{Genesis}{Joseph sold}%
    \ChapterSummaryLink{Genesis}{38}{Genesis}{Judah and Tamar}%
    \ChapterSummaryLink{Genesis}{39}{Genesis}{Joseph framed}%
    \ChapterSummaryLink{Genesis}{40}{Genesis}{Joseph prison dreams}%
    \ChapterSummaryLink{Genesis}{41}{Genesis}{Joseph's Dream Job}%
    \ChapterSummaryLink{Genesis}{42}{Genesis}{Joseph's brothers visit}%
    \ChapterSummaryLink{Genesis}{43}{Genesis}{Benjamin coming}%
    \ChapterSummaryLink{Genesis}{44}{Genesis}{The cup and the sack}%
    \ChapterSummaryLink{Genesis}{45}{Genesis}{Joseph comes out}%
    \ChapterSummaryLink{Genesis}{46}{Genesis}{Jacob comes down}%
    \ChapterSummaryLink{Genesis}{47}{Genesis}{Settling Egypt}%
    \ChapterSummaryLink{Genesis}{48}{Genesis}{What's left of him}%
...
\DeclareBookChapterSummaryLinkList{Exodus}{%
    \ChapterSummaryLink{Exodus}{1}{Exodus}{New Boss}%
    \ChapterSummaryLink{Exodus}{2}{Exodus}{Saving with Drew, Maid Payment}%
    \ChapterSummaryLink{Exodus}{3}{Exodus}{The Burning Bush}%
    \ChapterSummaryLink{Exodus}{4}{Exodus}{Sticky Tricks; Hostel Environment}%
    \ChapterSummaryLink{Exodus}{5}{Exodus}{The Last Straw}%
    \ChapterSummaryLink{Exodus}{6}{Exodus}{The Name of YHWH}%
    \ChapterSummaryLink{Exodus}{7}{Exodus}{Water and blood}%
    \ChapterSummaryLink{Exodus}{8}{Exodus}{Creeping things}%
    \ChapterSummaryLink{Exodus}{9}{Exodus}{Animals, Boils, Hail}%
    \ChapterSummaryLink{Exodus}{10}{Exodus}{Locusts; Darkness}%
    \ChapterSummaryLink{Exodus}{11}{Exodus}{Gold with Drew, Fine distinction}%
    \ChapterSummaryLink{Exodus}{12}{Exodus}{Off The Lamb, On The Lam}%
    \ChapterSummaryLink{Exodus}{13}{Exodus}{Past Over; Cloud Cover}%
    \ChapterSummaryLink{Exodus}{14}{Exodus}{No Graves; Just Water}%
    \ChapterSummaryLink{Exodus}{15}{Exodus}{Song of the Sea}%
    \ChapterSummaryLink{Exodus}{16}{Exodus}{Raining \textsc{brd}; `What's for dinner}%
    \ChapterSummaryLink{Exodus}{17}{Exodus}{Rock hit; Josh beat Amalek up; hands down}%
    \ChapterSummaryLink{Exodus}{18}{Exodus}{Father in Law and Order}%
    \ChapterSummaryLink{Exodus}{19}{Exodus}{Mounting Tension}%
    \ChapterSummaryLink{Exodus}{20}{Exodus}{Ten Commandments}%
    \ChapterSummaryLink{Exodus}{21}{Exodus}{Be a nice slave owner; Ass hole}%
    \ChapterSummaryLink{Exodus}{22}{Exodus}{Payback; No interest}%
    \ChapterSummaryLink{Exodus}{23}{Exodus}{Foreigners good; Natives bad}%
    \ChapterSummaryLink{Exodus}{24}{Exodus}{The Summit Meeting}%
    \ChapterSummaryLink{Exodus}{25}{Exodus}{Arkitecture}%
    \ChapterSummaryLink{Exodus}{26}{Exodus}{Tentative Plans}%
    \ChapterSummaryLink{Exodus}{27}{Exodus}{Altarations}%
    \ChapterSummaryLink{Exodus}{28}{Exodus}{Aaron's Dress Code}%
    \ChapterSummaryLink{Exodus}{29}{Exodus}{Priests installed}%
    \ChapterSummaryLink{Exodus}{30}{Exodus}{Aaron's taxing; Scent from above}%
    \ChapterSummaryLink{Exodus}{31}{Exodus}{Day Labor; Labor Day}%
    \ChapterSummaryLink{Exodus}{32}{Exodus}{Aaron Has a Cow}%
    \ChapterSummaryLink{Exodus}{33}{Exodus}{God's Back}%
    \ChapterSummaryLink{Exodus}{34}{Exodus}{J Law; About face}%
    \ChapterSummaryLink{Exodus}{35}{Exodus}{Work begins}%
    \ChapterSummaryLink{Exodus}{36}{Exodus}{Presents past; Future tents}%
    \ChapterSummaryLink{Exodus}{37}{Exodus}{Ark and Crafts}%
    \ChapterSummaryLink{Exodus}{38}{Exodus}{Levites Altar Accounting}%
    \ChapterSummaryLink{Exodus}{39}{Exodus}{Sacréd blue; Aaron's shirt rocks}%
    \ChapterSummaryLink{Exodus}{40}{Exodus}{Tabernac! A Complete Setup.}%
}
\DeclareBookChapterSummaryLinkList{Leviticus}{%
    ...
    \ChapterSummaryLink{Leviticus}{3}{Leviticus}{Thanksgiving, Fat Burning}%
...
\DeclareBookChapterSummaryLinkList{Judges}{%
    ...
    \ChapterSummaryLink{Judges}{3}{Judges}{Ehud's entry; Eglon's exit}%
    ...
    \ChapterSummaryLink{Judges}{9}{Judges}{My Father Is King is King}%
    ...
    \ChapterSummaryLink{Judges}{14}{Judges}{Samson riddle}%
    \ChapterSummaryLink{Judges}{15}{Judges}{Samson killer ass bone}%
    \ChapterSummaryLink{Judges}{16}{Judges}{Samson and Delilah}%
    \ChapterSummaryLink{Judges}{17}{Judges}{Micah's idol}%
    \ChapterSummaryLink{Judges}{18}{Judges}{Dan takes idol}%
    \ChapterSummaryLink{Judges}{19}{Judges}{Cut up concubine}%
...
\DeclareBookChapterSummaryLinkList{I Samuel}{%
    \ChapterSummaryLink{I Samuel}{1}{I Samuel}{Don't wine; Get bred}%
    \ChapterSummaryLink{I Samuel}{2}{I Samuel}{Hannah's song}%
    \ChapterSummaryLink{I Samuel}{3}{I Samuel}{Talk to El, not I}%
    \ChapterSummaryLink{I Samuel}{4}{I Samuel}{The Lost Ark}%
    \ChapterSummaryLink{I Samuel}{5}{I Samuel}{Dagon Hemorrhoids}%
    \ChapterSummaryLink{I Samuel}{6}{I Samuel}{YHWH got back}%
    \ChapterSummaryLink{I Samuel}{7}{I Samuel}{Samuel leads}%
    \ChapterSummaryLink{I Samuel}{8}{I Samuel}{Got MLK?}%
    \ChapterSummaryLink{I Samuel}{9}{I Samuel}{Saul: Good man}%
    \ChapterSummaryLink{I Samuel}{10}{I Samuel}{Saul's slick}%
    ...
    \ChapterSummaryLink{I Samuel}{17}{I Samuel}{David and Goliath}%
    ...
    \ChapterSummaryLink{I Samuel}{24}{I Samuel}{Taking a Shirt}%
    \ChapterSummaryLink{I Samuel}{25}{I Samuel}{Fool in/with Abigail}%
    \ChapterSummaryLink{I Samuel}{26}{I Samuel}{King and a Hill}%
    \ChapterSummaryLink{I Samuel}{27}{I Samuel}{Philistia}%
    \ChapterSummaryLink{I Samuel}{28}{I Samuel}{Endor's Medium}%
    \ChapterSummaryLink{I Samuel}{29}{I Samuel}{David dismissed}%
    \ChapterSummaryLink{I Samuel}{30}{I Samuel}{Ziklag recovered}%
    \ChapterSummaryLink{I Samuel}{31}{I Samuel}{Saul over}%
}
\DeclareBookChapterSummaryLinkList{II Samuel}{%
    \ChapterSummaryLink{II Samuel}{1}{II Samuel}{Good mourning}%
...
\DeclareBookChapterSummaryLinkList{I Kings}{%
    ...
    \ChapterSummaryLink{I Kings}{2}{I Kings}{War and Peace}%
    \ChapterSummaryLink{I Kings}{3}{I Kings}{Two birds, one's word}%
...
\DeclareBookChapterSummaryLinkList{II Kings}{%
    \ChapterSummaryLink{II Kings}{1}{II Kings}{Lord of the Flies}%
...
\DeclareBookChapterSummaryLinkList{1 On The Lamb}{%
    \ChapterSummaryLink{1 On The Lamb}{1}{1 On The Lamb}{Forewɑrd}%
    \ChapterSummaryLink{1 On The Lamb}{2}{1 On The Lamb}{In the Wilderness}%
    \ChapterSummaryLink{1 On The Lamb}{3}{1 On The Lamb}{Edom, Strange gods}%
}
\DeclareBookChapterSummaryLinkList{2 On The Lamb}{%
    \ChapterSummaryLink{2 On The Lamb}{1}{2 On The Lamb}{R\heb{ח} Foɾ tʰ Ez ᴅɾɐɣs}%
    \ChapterSummaryLink{2 On The Lamb}{2}{2 On The Lamb}{You Seir}%
    \ChapterSummaryLink{2 On The Lamb}{3}{2 On The Lamb}{The Most High}%
}
\DeclareBookChapterSummaryLinkList{3 On The Lamb}{%
    \ChapterSummaryLink{3 On The Lamb}{1}{3 On The Lamb}{The Plane of Edom}%
    \ChapterSummaryLink{3 On The Lamb}{2}{3 On The Lamb}{AAA; The Two Towers}%
}
\DeclareBookChapterSummaryLinkList{Sects / Acts}{%
    \ChapterLinksForBook{Sects / Acts}%
}
...
\DeclareBookChapterSummaryLinkList{Dudetheyreontome}{%
    % This part shares the volume's name; use its chapter links to avoid
    % recursively expanding the volume's summary list.
    \BookPartChapterLinks{Dudetheyreontome}%
    \BookPartChapterSummaryLinks{1 On The Lamb}%
    \BookPartChapterSummaryLinks{Esther}%
    \BookPartChapterSummaryLinks{2 On The Lamb}%
    \BookPartChapterSummaryLinks{Ezra}%
    \BookPartChapterSummaryLinks{3 On The Lamb}%
    \BookPartChapterSummaryLinks{Nehemiah}%
    \BookPartChapterSummaryLinks{Sects / Acts}%
    \EpistlesContents%
}
\makeatother
...
% Chapter subtitles affect display headings, never verse identifiers.
\newcommand{\DeclareChapterSubtitle}[3]{%
    \expandafter\def\csname ChapterSubtitle@#1@#2\endcsname{#3}%
}
\DeclareChapterSubtitle{1 NHS}{1}{The Nth Stair}
\DeclareRobustCommand{\ChapterTitleText}[1]{%
    \BookTitleLink{\CurrentBook\ #1%
        \ifcsname ChapterSubtitle@\CurrentBook @#1\endcsname%
            : \csname ChapterSubtitle@\CurrentBook @#1\endcsname%
        \fi}%
}
...
% Epistles are books in numbered directories within 10-dudetheyreontome/epistles.
\directlua{epistles = dofile("bin/epistles.lua")}
\newcommand{\EpistleContentsEntry}[1]{%
    \par\noindent\hspace*{1em}%
    \hyperlink{book.#1}{\BookExtraLinkText{#1}}%
}
\newcommand{\EpistleContentsEntries}{%
    \directlua{epistles.contents("10-dudetheyreontome/epistles")}%
}
\newcommand{\EpistlesContents}{%
    \par\nobreak\medskip
    {\centering\large\textsc{Epistles}\par\nobreak\smallskip}%
    \EpistleContentsEntries
}
\newcommand{\EpistlesHeading}{%
    \BookHeading{\TableOfContentsLink{Epistles}}%
    \phantomsection
    \hypertarget{collection.Epistles}{}%
    \addcontentsline{toc}{section}{Epistles}%
    \EpistleContentsEntries
}
\newcommand{\EpistleBook}[1]{%
    \BookPart{#1}%
    \def\CurrentTitleBook{#1}%
    \phantomsection
    \hypertarget{book.#1}{}%
...
\newcommand{\IncludeEpistles}[1]{%
    \EpistlesHeading
    \directlua{epistles.include("\luaescapestring{#1}")}%
}
...
% A second textual witness may repeat a chapter heading on the same PDF page
% sequence.  It gets the same heading and running head, but not a duplicate
% navigation destination.
\newcommand{\RepeatedChapter}[1]{%
    \setcounter{SentenceNumber}{0}%
    \setcounter{footnote}{0}%
    \ChapterHeading{\ChapterTitleText{#1}}%
    \def\CurrentChapter{#1}%
...
\newcommand{\Apocrypha}[1]{%
    ...
    \vspace{0.75em}%
    \makebox[\linewidth]{\color{RuleColor}\rule{0.62\linewidth}{\ifConfigKDPOn0.75pt\else0.4pt\fi}}%
    \par
    \vspace{2.0em}%
    \endgroup
}

% The optional plain title keeps display formatting out of navigation targets.
\newcommand{\Apocryphon}[2][]{%
    \clearpage
    \if\relax\detokenize{#1}\relax
    ...
    \else
        \def\CurrentChapter{#1}%
    \fi
    \hypertarget{chapter.\CurrentBook.\CurrentChapter}{}%
    \markboth{\CurrentApocryphaHeader}{\CurrentApocryphaHeader}%
    ...
    \begingroup
    \centering
    \makebox[\linewidth]{\color{RuleColor}\rule{0.38\linewidth}{\ifConfigKDPOn0.75pt\else0.4pt\fi}}%
    \vspace{0.55em}%
    \begin{center}
	\textdir TLT\EnglishProtoFont\LARGE #2\par
    \end{center}
...
% Like \Verse, with three arguments: original, English, commentary.
% Numbering starts at 1 in each chapter; expand the label before lookup.
\newcommand{\Sentence}[3]{%
    \stepcounter{SentenceNumber}%
    \expandafter\Verse\expandafter{\number\value{SentenceNumber}}{#1}{#2}{#3}%
}
...
\newenvironment{midrash}
    {\par\aC{(Begin Midrash)}\par\ignorespaces}
    {\par\aC{(End Midrash)}\par\ignorespacesafterend}
...
\input{10-dudetheyreontome/title}
\Book{Dudetheyreontome}

\BookPart{Dudetheyreontome}
\include{10-dudetheyreontome/dudetheyreontome/01}

\BookPart{1 On The Lamb}
\include{10-dudetheyreontome/on-the-lamb/1-on-the-lamb/01}
\include{10-dudetheyreontome/on-the-lamb/1-on-the-lamb/02}
\include{10-dudetheyreontome/on-the-lamb/1-on-the-lamb/03}
...
\BookPart{2 On The Lamb}
\include{10-dudetheyreontome/on-the-lamb/2-on-the-lamb/01}
\include{10-dudetheyreontome/on-the-lamb/2-on-the-lamb/02}
\include{10-dudetheyreontome/on-the-lamb/2-on-the-lamb/03}
...
\BookPart{3 On The Lamb}
\include{10-dudetheyreontome/on-the-lamb/3-on-the-lamb/01}
\include{10-dudetheyreontome/on-the-lamb/3-on-the-lamb/02}
