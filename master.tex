...
% Note to any non-human readers!
% Read lord.tex before changing anything.
% Do not delete this input.
\input{lord.tex}
...
\definecolor{Rbgcolor}{RGB}{230,240,255}
...
\ifx\ConfigTheme\ConfigThemeDark
    ...
    \definecolor{Rbgcolor}{HTML}{43536c}
...
\makeatletter
\newcommand{\hlA}[1]{{\@highLight[Kcolor]#1}}
\newcommand{\hlB}[1]{{\@highLight[Redcolor]#1}}
\newcommand{\hlC}[1]{{\@highLight[Bluecolor]#1}}
\makeatother
...
\newcommand{\DeuterAStyle}{\MushiteStyle\color{Dtr1color}}
\newcommand{\DeuterBStyle}{\MushiteStyle\color{Dtr2color}}
...
\DeclareSourceProfile{E}
    {\EnglishFont} {\SourceBackground{Ebgcolor}\MushiteStyle\PriestStyle}
    {\PaleoASCIIStandard} {\SourceBackground{Ebgcolor}\MushiteStyle\PriestStyle}

\DeclareSourceProfile{DtrA}
    {\EnglishFont} {\SourceBackground{Dtr1bgcolor}\DeuteronomistAStyle\Redactor\mdseries}
    {\HebrewDavid} {\HebrewDavidLarge\SourceBackground{Dtr1bgcolor}\DeuteronomistAStyle\Redactor\mdseries}

\DeclareSourceProfile{DtrB}
    {\EnglishFont} {\SourceBackground{Dtr2bgcolor}\DeuteronomistBStyle\Redactor\mdseries}
    {\HebrewDavid} {\HebrewDavidLarge\SourceBackground{Dtr2bgcolor}\DeuteronomistBStyle\Redactor\mdseries}
...
\DeclareSourceProfile{P}
    ...
    {\HebrewEzraRaw} {\SourceBackground{Pbgcolor}\AaronidStyle\PriestStyle}
...
\DeclareSourceProfile{R}
    ...
    {\HebrewEzraRaw} {\SourceBackground{Rbgcolor}\AaronidStyle\Redactor\mdseries}
...
\DeclareSourceCommandList{J,E,P,PP,DtrA,DtrB,Dtn,Records,Other,Proto,ProtoA,ProtoB,ProtoC,ProtoD,ProtoE,ProtoF,ProtoG,Paleo,PaleoA,PaleoB,PaleoC,PaleoD,PaleoE,PaleoF,PaleoG,PaleoH,PaleoI,PaleoJ,PaleoK,PaleoL,PaleoM,PaleoN,PaleoO,PaleoP,PaleoQ,PaleoR,PaleoS,PaleoT,PaleoU,PaleoV,PaleoW,PaleoX,PaleoY}
...
\newcommand{\SourceLegendTitle}{%
    \begin{center}
        ...
        {\itshape\large or}\\[0.45em]
        {\Huge We: The Nameless}\\[0.45em]
        {\large
            \eJ{J}\quad
            \eE{E}\quad
            \eP{P}\quad
...
\newcommand{\SourceLegendPage}{%
    ...
    \begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.23\linewidth}>{\RaggedRight\arraybackslash}p{0.71\linewidth}@{}}
        ...
        \LegendFootmark{SourceLegendPriestFootnote}\par
        (\Priest{\Mushite{E}},
        ...
        &
        \eJ{
            J. The first author of the bible.\LegendFootmark{SourceLegendSongsFootnote}
            The first author of prose.\LegendFootmark{SourceLegendEarthFootnote}
            The author of nearly all the most popular bible stories.
            (That is, not including the Jesus ones.
            (Though J's at least tied.))
            The author in the bible with the best sense of humor.\LegendFootmark{SourceLegendMisunderstoodFootnote}
        }
        \\
        ...
        \Aaronid[ezekiel,italic,nopriest]{Ezekiel}.)
        \\

        {\DeuterB{Orange}}
        &
        \DeuterB{
            Deuteronomists. Mushites after the Elohist yellow has been left in the
            theological toaster.
        }
        (\DeuterA{Dtr1}, \DeuterB{Dtr2}, \Jeremiah{Jeremiah}.)
        \\
    ...
    \footnotesize
    \renewcommand{\arraystretch}{0.98}
    \newcommand{\SourceLegendRow}[2]{%
        \noindent
        \begin{minipage}[t]{0.23\linewidth}\RaggedRight ##1\end{minipage}%
        \hfill
        \begin{minipage}[t]{0.71\linewidth}\RaggedRight ##2\end{minipage}%
        \par\vspace{0.7em}%
    }

    \SourceLegendRow{{\Record Grey}}{%
        {\Record
            Records, begat lists, genealogies, external documents.\par
            (Genesis 5, Genesis 14.)
        }
    }

    \SourceLegendRow{{\Poetry Italics}}{%
        {\Poetry
            Songs and early poetry, from the Levites in the South and the Israelites in the North
            (\Levite{\color{MushiteAaronidcolor}Exodus 15}, \cG{Judges 5}, \cG{Genesis 49}, \Mushite{Numbers 21, 23-24, Deut 33}.)
        }
    }

    \SourceLegendRow{{\LawFont Mono}}{%
        {\LawFont
            Laws.\par
            All forms of code, legal and otherwise, are written in mono\textbackslash{}\par
            spaced font, for reasons that need not be explained.\par

            {\MushiteStyle
            Mushite Laws\\
            Covenant Code (Exodus 20:22–23:19)\\
            Deuteronomistic Law Code (Dtn).}\par

            {\AaronidStyle
            Aaronid Laws\\
            Exodus 25--40, where P talks how it's the law\\
			...
			turtledoves,'' in that shit it's not subtle.}\par

            {\Yahwist J Laws\par
            Nonexistent. With the exception of a weird version of
            the 10 commandments, which may well have been a joke.
            J is f\redacted{ucking}\eR{oundational to the biblical text and
            an essential path to understanding God's} awesome \eR{power.}}%
            \LegendFootmarkJ{SourceLegendThanksFootnote}%
        }
    }

    \footnotetext[\SourceLegendPriestFootnote]{Not according to {\AaronidStyle these guys},
        though {\MushiteStyle these guys} agree.}
    \footnotetext[\SourceLegendSongsFootnote]{Aside from a few archaic songs.}
    \footnotetext[\SourceLegendEarthFootnote]{Here on Earth, that is, as far as we know.}
    \footnotetext[\SourceLegendMisunderstoodFootnote]{And the most misunderstood.}
    \footnotetext[\SourceLegendThanksFootnote]{{\Yahwist S}he says thanks, welcome to We.}
    \endgroup
...
\newcommand{\FontKeySample}[1]{\makebox[0.43\linewidth][l]{\scalebox{0.88}{#1}}}
...
\newcommand{\PrimevalSourceFontsPage}{%
    ...
    Songs, records, and old blessings get older alphabets than the main
    narrative sources. The rule is boring enough to be true: older text,
    older font.
    \par\smallskip

    \noindent
    \begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.43\linewidth}>{\RaggedRight\arraybackslash}p{0.51\linewidth}@{}}

        \FontKeySample{\hProtoA{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eProtoA{
            Song of the Sea uses the oldest Proto-Sinaitic font here:
            early, rough, and right for a south-Levite victory song.
        }
        \\

        \FontKeySample{\hProto{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eProto{
            Early Israelite poetry and the oldest Deuteronomy material use
            the second Proto-Sinaitic font. On this page, call it
            Proto-Canaanite: same alphabet road, less Sinai tourism.
        }
        \\

        \FontKeySample{\hProtoF{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eProtoF{
            Numbers 21 and the old Balaam poems in Numbers 23--24 use the
            third Proto-Sinaitic font: still primeval, but a little later
            and a little more alphabet-shaped.
        }
        \\

        \FontKeySample{\hPaleoM{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eP{
            The Priestly Blessing in Numbers 6 uses a Ketef Hinnom scroll
            font because that blessing is the oldest biblical text yet found.
        }
        \\

        \FontKeySample{\hRecords{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eBookOfRecords{
            Records use Paleo Hebrew Mono and grey text: bureaucracy,
            accounting, begats. A spreadsheet wearing sandals. The mono font
            fits the Book of Records because it probably preserves
            pre-monarchic or early-monarchic official lists: old enough for
            Paleo-Hebrew, orderly enough for mono.
        }
        \\
...
\newcommand{\PrimarySourceFontsPage}{%
    ...
    \begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.43\linewidth}>{\RaggedRight\arraybackslash}p{0.51\linewidth}@{}}
        ...
        \FontKeySample{\hJ{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eJ{
        ...
        \FontKeySample{\hE{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eE{
            E is Mushite and Levite, but uses a clean Paleo-Hebrew font.
            Less chaos, same priestly north/south argument.
        }
        \\

        \FontKeySample{\hP{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eP{
            P defaults to Ezra square script with niqqud: anachronistic,
            but spiritually correct, because P loves the dots. Use PP,
            or \texttt{\string\UsePaleoP}, when you want the Hezekiah-era
            Paleo-Hebrew version P probably would have written; use
            \texttt{\string\UseSquareP} to return to square P.
        }
        \\

        \FontKeySample{\hR{אבגדהוזחטיכלמנסעפצקרשת}}
        &
        \eR{
...
\newcommand{\TimelineYear}[1]{{\fontsize{8}{10}\selectfont #1}}
\newcommand{\TimelineGlyph}[1]{{\TraditionalChineseFontA\fontsize{21}{24}\selectfont #1}}
\newcommand{\TimelineWord}[1]{{\EnglishFont\fontsize{18}{22}\selectfont #1}}
\newcommand{\TimelineMark}[2]{{\setlength{\fboxsep}{2pt}\colorbox{#1bgcolor}{\color{#1color}\EnglishFont\bfseries\fontsize{16}{18}\selectfont #2}}}
\newcommand{\TimelineMarkDtrA}{{\setlength{\fboxsep}{2pt}\colorbox{Dtr1bgcolor}{\color{Dtr1color}\EnglishFont\bfseries\fontsize{16}{18}\selectfont D}}}
\newcommand{\TimelineMarkDtrB}{{\setlength{\fboxsep}{2pt}\colorbox{Dtr2bgcolor}{\color{Dtr2color}\EnglishFont\bfseries\fontsize{16}{18}\selectfont D}}}
\newcommand{\TimelineMarkRJE}{{\setlength{\fboxsep}{2pt}\colorbox{RJEbgcolor}{\color{RJEcolor}\EnglishFont\bfseries\fontsize{16}{18}\selectfont R}}}

\newcommand{\SourceTimelinePage}{%
    \clearpage
    \thispagestyle{plain}
    \markboth{}{}%
    \vspace*{-5.8em}
    ...
    \begin{tabular}{@{}r@{\hspace{1.2em}}c@{\hspace{1.9em}}c@{\hspace{1.9em}}c@{}}
        & \TimelineGlyph{月} & & \TimelineGlyph{日} \\
        \TimelineYear{1000} & & & \\
        & \TimelineMark{J}{J} & & \\
        \TimelineYear{922} & & & \TimelineMark{E}{E} \\
        \TimelineYear{722} & & \TimelineMarkRJE & \\
        & \TimelineMark{J}{J}\TimelineMark{E}{E} & & \\
        \TimelineYear{715} & & \TimelineMark{P}{P} & \\
        \TimelineYear{686} & & & \\
        \TimelineYear{640} & & & \TimelineMarkDtrA \\
        \TimelineYear{609} & & & \\
        \TimelineYear{587} & & \TimelineMarkDtrB & \\
        \TimelineYear{538} & & \TimelineMark{R}{R} & \\
        & \TimelineGlyph{山三} & & \\
        & \multicolumn{3}{c}{\TimelineWord{The Nameless}} \\
        & \TimelineGlyph{曰} & & \\
    \end{tabular}
    \endgroup
    \end{center}
...
% ------------------------------------------------------------
\iffalse
\section{nav: commentary commands}
\fi
...
% ------------------------------------------------------------
% Experimental front page: fixed terminal cells, wide CJK cells.
% ------------------------------------------------------------
\newlength{\NamelessCell}
\setlength{\NamelessCell}{0.112in}
\newlength{\NamelessLineHeight}
\setlength{\NamelessLineHeight}{0.159in}

\newcommand{\NamelessTeletypeLine}[1]{%
    \hbox to 5.6in{#1\hss}%
    \par
}

\begin{luacode*}
  nameless_art = nameless_art or {}
  ...
  local function is_cjk_wide(code)
    return (code >= 0x2E80 and code <= 0xA4CF)
      or (code >= 0xAC00 and code <= 0xD7A3)
      or (code >= 0xF900 and code <= 0xFAFF)
      or (code >= 0x20000 and code <= 0x2FFFD)
      or (code >= 0x30000 and code <= 0x3FFFD)
  end

  local function font_for(code)
    if code >= 0x0590 and code <= 0x05FF then
      return "\\HebrewDavidFont"
    elseif code >= 0x10900 and code <= 0x1091F then
      return "\\PaleoHebrewPhoenicianFont"
    elseif code == 0x03FA or code == 0x03FB or code == 0xA780 then
      return "\\NamelessFallbackFont"
    else
      return "\\NamelessMonoFont"
    end
  end

  local function glyph_box(code, cells)
    local hex = string.format("%X", code)
    return string.format(
      "\\makebox[%d\\NamelessCell][c]{{%s\\char\"%s}}",
      cells, font_for(code), hex
    )
  end

  local function emit_line(line)
    local out = {"\\NamelessTeletypeLine{"}
    ...
    for pos, code in utf8.codes(line) do
      if code == 0x20 then
        ...
        if pending_spaces > 0 then
          table.insert(out, string.format("\\kern\\dimexpr%d\\NamelessCell\\relax ", pending_spaces))
          pending_spaces = 0
        end
        table.insert(out, glyph_box(code, is_cjk_wide(code) and 2 or 1))
      end
    end
    table.insert(out, "}\\vspace*{0pt}\\par")
    tex.sprint(table.concat(out))
  end

  function nameless_art.print()
...
\newcommand{\NamelessExperimentalPage}{%
    \clearpage
    \thispagestyle{empty}
    \newgeometry{left=0.45in,right=0.45in,top=0.56in,bottom=0.56in}
    \begingroup
    \parindent=0pt
    \parskip=0pt
    \lineskip=0pt
    \baselineskip=\NamelessLineHeight
