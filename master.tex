...
\newcommand{\ChapterSummaryLink}[4]{%
    \par\noindent
    \hyperlink{chapter.#1.#2}{\ChapterSummaryLinkText{#3\ #2 --- #4}}%
}
\newcommand{\ChapterSummaryLinksForBook}[1]{%
    \begingroup
    \def\CurrentBook{#1}%
    \directlua{PrintChapterSummaryLinksForBook("\luaescapestring{#1}")}%
    \endgroup
}
...
\begin{luacode*}
    local toc_path = "TOC.md"
    local resolved_path = kpse.find_file(toc_path) or toc_path
    local toc = io.open(resolved_path, "r")
    ...
    local ordered_books = {}
    local book_summaries = {}

    local function current_book_name(name)
        return book_alias[name] or name
    end
    ...
    if toc then
        ...
        local current_display_book = nil
        for line in toc:lines() do
            local heading = line:match("^##%s+(.+)$")
            if heading then
                current_book = current_book_name(heading)
                current_display_book = heading
                if not book_summaries[current_book] then
                    table.insert(ordered_books, current_book)
                    book_summaries[current_book] = {}
                end
            else
                local source_book, chapter, summary =
                    line:match("^%- (.-) (%d+) " .. dash .. " (.-)%.?$")
                if current_book and chapter and summary then
                    table.insert(book_summaries[current_book], {
                        chapter = chapter,
                        display_book = source_book or current_display_book,
                        summary = summary,
                    })
                end
            end
        ...
        texio.write_nl("term and log", "Warning: could not open " .. toc_path)
    end

    function PrintChapterSummaryLinksForBook(book)
        local items = book_summaries[book]
        if not items then
            tex.sprint("\\ChapterLinksForBook{" .. tex_escape(book) .. "}")
            return
        end
        for _, item in ipairs(items) do
            tex.sprint(
                "\\ChapterSummaryLink{" .. tex_escape(book) .. "}{" ..
                item.chapter .. "}{" .. tex_escape(item.display_book) .. "}{" ..
                tex_escape(item.summary) .. "}"
            )
        end
    end
\end{luacode*}
