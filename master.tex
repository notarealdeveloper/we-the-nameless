...
\def\ConfigOutputModePrint{print}
\def\ConfigOutputModePhone{phone}
...
\fi
\ifx\ConfigOutputMode\ConfigOutputModePhone
    \ConfigOutputModePhonetrue
\else
    \ConfigOutputModePhonefalse
\fi
...
% Older chapter sources use this name when temporarily restoring the compact
% print-size ladder inside larger-format sections.
\let\BookModeTextSizes\PrintModeTextSizes

\newcommand{\PhoneModeTextSizes}{%
...
\ifConfigOutputModePhone
% Phone keeps the same typographic ladder as 11pt print mode, scaled together.
...
\definecolor{PhoneLinkColor}{HTML}{8ec7ff}
...
\ifx\ConfigTheme\ConfigThemeDark
    ...
    \colorlet{ContentsLinkColor}{PhoneLinkColor}
\else
...
\newcount\SourceFontLineBreakPhonePenalty
\SourceFontLineBreakPhonePenalty=3000
\newcommand{\SourceFontLineBreak}{%
    \ifConfigOutputModePhone
        \ifhmode
            \unskip
            \penalty\SourceFontLineBreakPhonePenalty
...
\newcommand{\AlphabetHistoryTextSizes}{%
    % Use one fixed canvas in both output modes.  At the ordinary print-mode
    ...
    % produce the same clean alignment that phone mode has.
