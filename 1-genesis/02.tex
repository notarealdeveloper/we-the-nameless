...
\Def{Eden}{
(\textit{Sumerian}: \textit{eden}). Sumerian term meaning ``plain,'' ``steppe,'' or ``open uncultivated land,'' widely attested across Mesopotamian history in both phonetic and logographic forms.
}

\Def{Sumerian Poems}{
Early Mesopotamian literary traditions preserving myths, royal narratives, and political struggles; includes tales such as \textit{The Envoys of Agga}, where Uruk resists the authority of Kish.
}

\Def{Pishon}{
(Heb, \iheb{פישון}). Possibly from a root meaning ``to spring forth,'' ``spread out,'' or ``break loose'' (\iheb{פוש}/\iheb{פיש}); evocative of flowing, branching, or overflowing motion.
}
...
\Def{Assyria}{
Major ancient Mesopotamian civilization, originating as a city-state and later becoming one of the dominant empires of the ancient Near East.
}
