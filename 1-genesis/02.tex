...
\Def{Pishon}{
(Heb, \heb{פישון}).\\
Unknown meaning. Possibly from a root meaning ``to spring forth.'' Only used once in the Bible, in Genesis 2:11, and not attested elsewhere.
}
...
\Def{PSHN}{
(Heb, \heb{פשנ}).\\
A meaningless consonant cluster, appearing to be a misspelling of Pishon, with an orthogrgaphically invalid non-terminal Nun (\heb{נ}), and an omission of the matres lectiones \heb{י} and \heb{ו}. It is unclear why this is being defined.
}
...
Consonant letters used to indicate vowels in Semitic writing systems. In Hebrew these are primarily \heb{א}, \heb{ה}, \heb{ו}, and \heb{י}. The latter two especially often function as vowel markers rather than full consonants. Their use helps disambiguate pronunciation in scripts that originally recorded mainly consonants.
Thus a form like \heb{פשנ} may be viewed as a \textit{defective spelling} of something expected to include the matres \heb{י} or \heb{ו}, as in \heb{פישון}. Similar phenomena occur across Hebrew, Aramaic, Phoenician, Moabite, and Ugaritic orthographic traditions, especially in older writing.
}
...
and \textit{napištum} (\akk{𒍣}, “life”), along with Arabic \arb{نَفْس} (\textit{nafs}, “soul, self, life”).\\
Meanings include: life (the state of being alive, of living), a person (human being), a soul, spirit (of a person or animal), or (archaic) breath. Frequently associated in Biblical Hebrew with living beings, desire, hunger, mortality, and the transition between breath and life. In Genesis 2:7 humanity becomes a \heb{נפש חיה} (“living nefesh”).
}
...
\def\protoaupper{{\textdir TRT ABGSHWZXjyKILMNOPCQRDUVT}}
\def\protoalower{{\textdir TRT abgshwzxjYklmnopcqruvt}}
\def\protobupper{{\textdir TRT ABGDHWZXJYKILMNSOPCQRVTCEFU}}
\def\protoblower{{\textdir TRT abgdhwzxjyiklmnsopcqrvtcefu}}
\def\paleoupper{{\textdir TRT  ABGDHWZXJYKILMNSOPCQRVTCEFU}}
\def\paleolower{{\textdir TRT  abgdhwzxjyiklmnsopcqrvtcefu}}
\def\hebrewalphabet{\heb{אבגדהוזחטיכלמנסעפצקרשת}}
Proto A upper: \protoa{\protoaupper}\\
Proto A lower: \protoa{\protoalower}\\
Proto B upper: \protob{\protobupper}\\
Proto B lower: \protob{\protoblower}\\
%Paleo A upper: \paleoa{\upper}\\
Paleo A lower: \paleoa{\paleolower}\\
Paleo B upper: \paleob{\paleoupper}\\
Paleo B lower: \paleob{\paleolower}\\
Paleo C upper: \paleoc{\paleoupper}\\
Paleo C lower: \paleoc{\paleolower}\\
Paleo D upper: \paleod{\paleoupper}\\
Paleo D lower: \paleod{\paleolower}\\
Paleo E upper: \paleoe{\paleoupper}\\
Paleo E lower: \paleoe{\paleolower}\\
Paleo F upper: \paleof{\paleoupper}\\
Paleo F lower: \paleof{\paleolower}\\
Paleo G upper: \paleog{\paleoupper}\\
Paleo G lower: \paleog{\paleolower}\\
Paleo H upper: \paleoh{\paleoupper}\\
Paleo H lower: \paleoh{\paleolower}\\
Paleo I upper: \paleoi{\paleoupper}\\
Paleo I lower: \paleoi{\paleolower}\\
Paleo J upper: \paleoj{\paleoupper}\\
Paleo J lower: \paleoj{\paleolower}\\
Paleo K upper: \paleok{\paleoupper}\\
Paleo K lower: \paleok{\paleolower}\\
Paleo T upper: \paleot{\paleoupper}\\
Paleo T lower: \paleot{\paleolower}\\
Hebrw J upper: \hebrewj{\hebrewalphabet}\\
Hebrw P upper: \hebrewp{\hebrewalphabet}\\

\Def{Havilah}{
(Heb, \heb{חוילה}).\\
Possibly related to roots for twisting, writhing, circling, labor pains, or linked chains (\heb{חול}, \heb{חיל}, \heb{חוליה}); strongly serpent- and motion-themed imagery.
}
...
\Def{Gold}{
(Heb, \heb{זהב}).\\
Cognate with Arabic \arb{ذهب}, meaning both (1) ``gold,'' and (2) ``to go,'' ``depart,'' ``vanish,'' or ``pass away,'' potentially implying something exiting, moving on, or leaving.
}

\Def{Bdellium}{
(Heb, \heb{בדלח}).\\
Possibly connected to roots meaning ``divide,'' ``separate,'' or ``scatter'' (\heb{בדל}); imagery of fragmentation, distinction, or pieces.
}

\Def{Onyx}{
(Heb, \heb{השהם}). Phonetically suggestive of \heb{השם} (``The Name,'' i.e.\ Yahweh) and \heb{אשם} (``guilt'' / ``guilt offering''), creating possible layered punning around divinity and culpability.
}
...
\Def{Gihon}{
(Heb, \heb{גיחון}). One of Eden's rivers; likely echoes roots of bursting forth or belly-like swelling, and later reappears at Solomon's coronation spring in Jerusalem.
}

\Def{Kish}{
(\textit{Kiš}). Ancient Mesopotamian city said in the Sumerian King List to be the first kingdom after the flood; associated with primordial kingship and post-diluvian order.
}

\Def{Kesh}{
(\textit{Keš}). Ancient Sumerian cult-city of Ninhursag, attested in Early Dynastic sources; associated with sacred urban federation traditions around Nippur.
}

\Def{Kush}{
(Heb, \heb{כוש}). Possibly evokes Semitic roots involving enclosure, concavity, stripping, or breaking; potentially punning with \heb{כוס} (``cup,'' later vulgar sexual slang), fitting Eden's winding river imagery.
}
...
\Def{Tigris}{
The eastern of Mesopotamia's two great rivers; one of the defining waterways of the ``land between the rivers.''
}

\Def{Euphrates}{
The longest and most historically significant river of Mesopotamia; together with the Tigris it defines the cradle-land between the rivers.
}
