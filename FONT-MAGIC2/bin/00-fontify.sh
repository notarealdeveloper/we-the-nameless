...
PYTHONPATH="$ROOT" python3 - "$ROOT/config/default.yaml" "$CONFIG" "$RUN/manifests/config.json" "$NAME" "$SCRIPT" "$ENCODING" "$OUTPUT" "$REPAIRS" "$MONO" "$PACKAGE" <<'PY'
...
def read_config(path):
 ...
 try: return json.loads(text)
  ...
  try: import yaml
  except ImportError as e: raise SystemExit('YAML syntax requires PyYAML; JSON syntax is accepted without it') from e
  return yaml.safe_load(text)
d=read_config(default)
if custom:
 c=read_config(custom)
 def merge(a,b):
