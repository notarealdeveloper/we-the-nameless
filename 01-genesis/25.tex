...
{
	\aB{Ok folks strap in, looks like we're in a begat list.}

	Ok so, remember how right after the b\eRJE{ind}ing of Isaac,

	J showed up with a begat list full of people with strangely dark names.

	Now E does the same thing.

	Side Note: This is the first we've heard from E since then.

	Here's the begat list.
	...
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Zimran}
			& \heb{זִמְרָן}
			& Probably ``musician / singer'' or ``song,'' from \heb{זמר}; uncertain.\\
		\hline
		\textbf{Jokshan}
			& \heb{יָקְשָׁן}
			& Uncertain; possibly related to a root meaning ``snare / trap.''\\
		\hline
		\textbf{Medan}
			& \heb{מְדָן}
			& Probably connected with \heb{דין}, ``judge / contend''; perhaps ``contention.''\\
		\hline
		\textbf{Midian}
			& \heb{מִדְיָן}
			& Probably ``strife / contention,'' connected with \heb{דין}.\\
		\hline
		\textbf{Ishbak}
			& \heb{יִשְׁבָּק}
			& ``He releases / leaves / abandons,'' from \heb{שׁבק}; better attested in Aramaic.\\
		\hline
		\textbf{Shuah}
			& \heb{שׁוּחַ}
			& Uncertain; possibly ``pit / depression'' or ``to sink / bow down.''\\
		\hline
	}
	\aB{New wife has a LOT of babies.}%
	\fC{One's name is Midian. Keep that in mind for later.}
...
}
{
	...
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Sheba}
			& \heb{שְׁבָא}
			& Uncertain ancient ethnonym;
			sometimes associated by wordplay with \heb{שבע},
			``seven,'' but not securely. \\
		\hline
		\textbf{Dedan}
			& \heb{דְּדָן}
			& Ancient Arabian place/people-name; meaning unknown. \\
		\hline
		\textbf{Ashurim}
			& \heb{אַשּׁוּרִם}
			& Probably an ethnonym; possibly related to Asshur/Assyria, but uncertain. \\
		\hline
		\textbf{Letushim}
			& \heb{לְטוּשִׁם}
			& Probably from \heb{לטשׁ}, ``hammer / sharpen,'' perhaps ``smiths / sharpeners'';
				 uncertain as an ethnonym. \\
		\hline
		\textbf{Leummim}
			& \heb{לְאֻמִּים}
			& Literally ``peoples / nations.'' \\
		\hline
	}

	\aB{
		Then Trap has two kids named
		\telugu{క్}evin\fB{
			Kid number seven in the chapter so far, interestingly.
		} and
...
}
{
	...
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Ephah}
			& \heb{עֵיפָה}
			& Probably ``darkness / gloom''; uncertain. \\
		\hline
		\textbf{Epher}
			& \heb{עֵפֶר}
			& ``Dust.'' \\
		...
		\textbf{Hanoch}
			& \heb{חֲנֹךְ}
			& ``Dedicated / initiated.'' Same name usually rendered Enoch. \\
		\hline
		\textbf{Abida}
			& \heb{אֲבִידָע}
			& ``Father knows'' / ``my father knows.'' \\
		\hline
		\textbf{Eldaah}
			& \heb{אֶלְדָּעָה}
			& ``God knows.'' \\
	...
	\aB{
		So now we reach the culmination of YHWH's idea to name Abe's kid
		laughter and then do a surprise bait and switch by prepending an
		S to his name and making Abe fulfill the prophecy.
	}

	Now Abe and Keturah\fB{
		Some manuscripts substitute the name Sonia Burnham.
	} have some kids named Conflict and Gloom and Initiated
	and Dad Knows and God Knows, like ya do.

	Side Note: Hey, remember how Abe's new wife is named Keturah?\fB{Ibid.}

	Her name derives from the Hebrew root qatar
	(\heb{קטר}), which means ``to burn a sacrifice.''

	Nothing to see here folks, don't worry about it, Isaac's fine
	\aC{in the sources in which he appears after Genesis 22,
		namely the non-E sources}.
...
}
{
	...
	Here a different source, likely the same editor who showed up in the middle
	of Genesis 22, appears again to clarify that Abe left Isaac everything in his will.
...
}
{
	\eRJE{
		...
		son, while he was still living:}%
	\fB{What's going on with that colon? I know it's not in the original Hebrew but what's that colon doing there? While "he" was still living? Which he?}%
	\eRJE{ east, to the land of the
		East.%
	}
}
{
	...
	\aB{Has Abe been spreading seed in private?}
}
...
{
	Abe lives 100 + 70 + 5 years in the life that he lived.
}

\Verse{8}
{
	\hP{וַיִּגְוַע}
	\hJ{וַיָּמת אַבְרָהָם בְּשֵׂיבָה טוֹבָה זָקֵן וְשָׂבֵעַ}
	\hP{וַיֵּאָסֶף אֶל עַמָּיו׃}
}
{
	\eP{And he expired.}
	\eJ{And Abraham died at a good old age, old and full,}
	\eP{and was gathered to his people.}
}
{
	Abe expires.

	He had a good long shelf life.

	Old and full.

	And he was gathered to his people.\footnote{Side Note: They used to have family tombs in the ancient near east. They'd bury four or five generations together and then seal it up and start a new one. So ``gathered to his people'' means literally ``buried with his parents and grandparents.'' They're pretty spread out though, so maybe here it's a metaphor.}
}
...
{
	\aB{Hey Ishmael's back!}

	\aB{Haha missed ya bro, God it's good to have you back hear.}
...
{
	He bought it from the Hittites, remember?

	Sarah was buried there, remember?
	
	And now the cave receives Abe too, and this is the receipt of his receipt.
}

\Verse{11}
{
	\hP{וַיְהִי אַחֲרֵי מוֹת אַבְרָהָם וַיְבָרֶךְ אֱלֹהִים אֶת יִצְחָק בְּנוֹ}
	\hJ{וַיֵּשֶׁב יִצְחָק עִם בְּאֵר לַחַי רֹאִי׃}
}
{
	\eP{And it was after Abraham’s death, and God blessed Isaac, his son.}
	\eJ{And Isaac lived at the well Lahai-roi.}
}
{
%	\footnote{
...
%	}
	...
	Side Note: Whenever there's a story with a sacrifice in it,
	P either deletes the sacrifice or deletes the story.\footnote{We'll find out why eventually.}

	Ok so Isaac lives at that well where his mom's sisterwife ran away to when his mom told her to go away.

	\aB{He lives there now.
...
}
{
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Nebaioth}
			& \heb{נְבָיֹת}
			& Uncertain; probably an Arabian tribal name. \\
		\hline
		\textbf{Kedar}
			& \heb{קֵדָר}
			& ``Dark / swarthy,'' from \heb{קדר}, ``be dark.'' \\
		\hline
		\textbf{Adbeel}
			& \heb{אַדְבְּאֵל}
			& Probably ``disciplined / chastened by El''; uncertain. \\
		\hline
		\textbf{Mibsam}
			& \heb{מִבְשָׂם}
			& ``Fragrance / perfume / spice.'' \\
		\hline
	}

	\aB{Ishmael has babies.

	We're not super sure what the first group's names mean.}
}
...
}
{
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Mishma}
			& \heb{מִשְׁמָע}
			& ``Hearing / thing heard,'' from \heb{שמע}, ``hear.'' \\
		\hline
		\textbf{Dumah}
			& \heb{דּוּמָה}
			& ``Silence.'' \\
		...
		\textbf{Massa}
			& \heb{מַשָּׂא}
			& ``Burden / load,'' or possibly ``oracle / utterance.'' \\
		\hline
	}

	\aB{Then he has some more.

	Their names are things like ``Hearing,'' and ``Silence,'' and ``Burden.''

	So the first one's like ``Ishmael Jr.''

	Then the next is a description of what he needs more of.

	Then the next is a description of what he needs less of.}
}
...
}
{
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Hadar}
			& \heb{חֲדַר}
			& Meaning uncertain; compare Hadad in some textual traditions. \\
		\hline
		\textbf{Tema}
			& \heb{תֵּימָא}
			& Arabian place/tribal name, Tayma; original meaning uncertain. \\
		\hline
		\textbf{Jetur}
			& \heb{יְטוּר}
			& Probably ``enclosure,'' or related to a root ``surround''; uncertain. \\
		\hline
		\textbf{Naphish}
			& \heb{נָפִישׁ}
			& Probably ``numerous / abundant''; compare Aramaic ``increase / spread.'' \\
		\hline
		\textbf{Kedmah}
			& \heb{קֵדְמָה}
			& ``Eastward / east,'' from \heb{קדם}. \\
		\hline
	}

	\aB{Toward the end he has a kid named ``So Many.''}
}
...
}
{
	...
	Though after you list the people, don't forget to list their names.
...
%	}
	...
	Oh and also before that they tent all the way from Havilah to Shur.
	...
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Havilah}
			& \heb{חֲוִילָה}
			& Uncertain; possibly connected with ``sand'' or a circular/district term. \\
		\hline
		\textbf{Shur}
			& \heb{שׁוּר}
			& Probably ``wall.'' \\
		...
		\textbf{Egypt}
			& \heb{מִצְרַיִם}
			& Mitzrayim; historical etymology uncertain. \\
		\hline
		\textbf{Asshur}
			& \heb{אַשּׁוּר}
			& Asshur / Assyria; originally city and god, then country; meaning uncertain. \\
		\hline
	}

	\aB{And he dies like, facing there.
...
%	}
	...
	Begat here d\eR{on't panic}\footnote{The text appears to have been corrupted here.} people.
}
...
}
{
	\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
		...
		\textbf{Isaac}
			& \heb{יִצְחָק}
			& ``He laughs / may he laugh.'' \\
		\hline
		\textbf{Rebekah}
			& \heb{רִבְקָה}
			& Uncertain; often connected with binding / tying, hence something like ``tether,''
				 but not secure. \\
		\hline
		\textbf{Bethuel}
			& \heb{בְּתוּאֵל}
			& Probably ``man / house of El'' or ``El's dwelling''; exact analysis disputed. \\
		\hline
		\textbf{Paddan Aram}
			& \heb{פַּדַּן אֲרָם}
			& Approximately ``plain / field of Aram''; Paddan probably ``field / plain.'' \\
		\hline
		\textbf{Laban}
			& \heb{לָבָן}
			& ``White.'' \\
		...
		\textbf{Aram}
			& \heb{אֲרָם}
			& Aram / Aramea; historical etymology uncertain. \\
		\hline
	}

	Ok so here we learn that Isaac married Rebecca, and he was 40 when that happened.

	\aB{Don't worry about how old she was according to the text when they married.}\fC{The traditional harmonizing calculation makes Rebecca three when she marries forty-year-old Isaac: Sarah is about ninety at Isaac's birth (Gen 17:17; 21:2, 5) and dies at 127 (23:1), making Isaac thirty-seven. If the notice of Rebecca's birth after the binding (22:20--23) is synchronized with Sarah's death, then Isaac's marriage at forty (25:20) follows three years later. See Rashi on Gen 25:20.}
}
...
}
{
	...
	Really it's just the main female characters, the supporting female characters are fertile the whole time.\fB{That's what makes them supporting.}
}
...
}
{
	\eJ{
		...
		\textsl{Two nations are in your womb,
...
}
{
	...
	\aC{
		Biblical Hebrew normally puts an imperfect (i.e., unfinished, ongoing action type) verb before its subject and object: ``serve \emph{older} \emph{younger}'' means ``the older will serve the younger.'' Here \emph{older} comes first: ``\emph{older} serve \emph{younger}.'' Both adjectives are masculine singular, and neither is marked as the object, so the words can mean either ``the older will serve the younger'' or ``the younger will serve the older.'' The traditional reading chooses the first, but the original author of the text here (J) appears to have carefully crafted this sentence to be deliberately ambiguous.
	}
...
}
{
	\aC{
		\Table{|p{0.18\linewidth}|p{0.16\linewidth}|p{0.56\linewidth}|}{
			...
			\textbf{Esau}
				& \heb{עֵשָׂו}
				& Etymology uncertain. Genesis associates him with hairiness at birth, but the
					 linguistic connection is unclear. \\
			\hline
		}
	}

	\aA{Ok so the linguistic connection here is really (p|f)unny and absolutely crystal clear.
	...
	\aB{The text says Esau is all hairy (Heb \heb{שער}, so basically he's all ``Seiry'').
	...
	\par\nopagebreak[4]
	\begin{center}
		\image[width=0.72\linewidth]{seir}
	\end{center}
	...
	}all three of these words are spelled exactly the same:
...
}
{
	...
	Everyone always comments on the \heb{עקב} part of the pun, but there's
	...
	\aB{He's called \paleo{𐤉𐤏𐤒𐤁} because he came out holding Esau's
	\paleo{𐤏𐤒𐤁}.
...
}
{
	...
	Which would be particularly ironic\fB{Aka, very J} given what's about to happen.
...
}
{
	...
	Sell me your birthright and I'll give you the soup.
}
...
{
	“No use have birthright if dead. Deal.” sayeth the Red Man to his bro.\fB{They're twins.}
}
...
}
{
	...
	\footnote{Or whatever he/it was called before the wrestling scene we haven't reached yet.}
}
